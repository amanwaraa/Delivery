...
const LOCAL_OFFLINE_DB_NAME = `${PREFIX}_offline_db_v3`;
const LOCAL_OFFLINE_DB_VERSION = 3;
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    alert("حدث خطأ أثناء تشغيل النظام. افتح Console لمعرفة التفاصيل.");
  }
});
...
function showExpired(message = "انتهى وقت المفتاح أو عدد الاستخدامات المتاحة.") {
  ...
  safeText(qs("expiredMessage"), message);
  lucide.createIcons();
}
...
async function updateCurrencyUI() {
  ...
  safeText(qs("sideCurrencyText"), `${settings.currencySymbol} ${settings.currencyName}`);
  safeText(qs("posCurrencyBadge"), `${settings.currencySymbol} ${settings.currencyName}`);
  safeText(qs("sideModeText"), settings.appMode === "offline" ? "أوفلاين" : "أونلاين");
  safeText(qs("setCurrentSystemMode"), settings.appMode === "offline" ? "أوفلاين" : "أونلاين");
...
function applyPlanBadgeFromSession() {
  ...
  safeText(qs("licensePlanBadge"), label);
  safeText(qs("settingsPlanBadge"), label);
}
...
function updateLicenseUIFromSession() {
  ...
  safeText(qs("sideLicenseKey"), session.key || "-");
  safeText(qs("sideLicenseRemaining"), formatRemaining(remaining));

  safeText(qs("setCurrentKey"), session.key || "-");
  safeText(qs("setCurrentLicenseType"), durationTypeLabel(session.durationType));
  safeText(qs("setCurrentLicenseStart"), formatDateTime(session.startedAt));
  safeText(qs("setCurrentLicenseEnd"), session.expiresAt ? formatDateTime(session.expiresAt) : "غير محدود");
  safeText(qs("setCurrentLicenseRemaining"), formatRemaining(remaining));
    ...
    grid.innerHTML += `
      <div class="card p-6 border-2 ${active ? "border-blue-500" : "border-transparent"}">
        <div class="flex items-center gap-4">
          ${logoHtml}
          <div class="flex-grow">
            <h4 class="font-bold text-lg">${escapeHtml(store.name || "بدون اسم")}</h4>
...
function fillProductForm(p = null) {
  safeValue(qs("editProductId"), p?.id || "");
  safeValue(qs("prodName"), p?.name || "");
  safeValue(qs("prodCode"), p?.code || "");
  safeValue(qs("prodStock"), p?.stock ?? "");
  safeValue(qs("prodCost"), p?.cost ?? "");
  safeValue(qs("prodPrice"), p?.price ?? "");
  renderVariantsForm(p?.variants || []);
}

function resetProductForm() {
  safeValue(qs("editProductId"), "");
  safeText(qs("modalTitle"), "إضافة منتج جديد");
  safeValue(qs("prodName"), "");
  safeValue(qs("prodCode"), "");
  safeValue(qs("prodStock"), "");
  safeValue(qs("prodCost"), "");
  safeValue(qs("prodPrice"), "");
  renderVariantsForm([]);
}
...
function showProductBarcode(code, title) {
  safeText(qs("barcodeTitle"), title || "باركود المنتج");
  safeText(qs("barcodeText"), code || "");
...
async function editProduct(id) {
  ...
  if (!p) return;
  safeText(qs("modalTitle"), "تعديل المنتج");
  fillProductForm(p);
  toggleModal("productModal", true);
}
...
function calculateTotal() {
  ...
  safeText(qs("subtotal"), money(sub));
  safeText(qs("discountPreview"), money(discountValue));
  safeText(qs("finalTotal"), money(total));
}

function updateCreateInvoiceButton() {
  safeText(qs("createInvoiceBtn"), editingInvoiceId ? "حفظ تعديل الفاتورة" : "إنشاء فاتورة");
}
...
async function getNextInvoiceNumber() {
  ...
  if (isOnline() && getLocalSession()?.appMode === "online" && baseClientPath()) {
    try {
...
function clearInvoiceEditor() {
  ...
  editingInvoiceId = null;
  renderCart();
  safeValue(qs("customerName"), "");
  safeValue(qs("customerPhone"), "");
  safeValue(qs("paymentMethod"), "cash");
  safeValue(qs("invoiceStatus"), "paid");
  safeValue(qs("invoiceNotes"), "");
  safeValue(qs("discountType"), "fixed");
  safeValue(qs("posDiscount"), 0);
  calculateTotal();
  updateCreateInvoiceButton();
...
async function editInvoice(id) {
  ...
  safeValue(qs("customerName"), inv.customer || "");
  safeValue(qs("customerPhone"), inv.phone || "");
  safeValue(qs("paymentMethod"), inv.payment || "cash");
  safeValue(qs("invoiceStatus"), inv.status || "paid");
  safeValue(qs("invoiceNotes"), inv.notes || "");
  safeValue(qs("discountType"), inv.discountType || "fixed");
  safeValue(qs("posDiscount"), Number(inv.discountRaw || 0));
...
function renderInvoiceBarcode(id) {
  ...
  try {
    ...
    safeText(qs("invoiceBarcodeText"), code);
  } catch {
    safeText(qs("invoiceBarcodeText"), code);
  }
}
...
async function viewInvoice(id) {
  ...
  safeText(qs("invPageStoreName"), store.name || "المحل");
  setImageOrHide(qs("invPageLogo"), store.logo);
  safeText(qs("invPageId"), `#${id}`);
  safeText(qs("invPageDate"), new Date(inv.date).toLocaleString("ar-EG"));
  safeText(qs("invPageCustomer"), inv.customer || "-");
  safeText(qs("invPagePhone"), inv.phone || "-");
  safeText(qs("invPagePayment"), inv.payment === "cash" ? "نقداً" : "إلكتروني");
  safeText(qs("invPageStatus"), statusLabel(inv.status || "paid"));
  ...
  safeText(qs("invPageSub"), `${Number(inv.subtotal || 0).toFixed(2)} ${inv.currencySymbol || "₪"}`);
  safeText(qs("invPageDiscount"), `${Number(inv.discount || 0).toFixed(2)} ${inv.currencySymbol || "₪"}`);
  safeText(qs("invPageTotal"), `${Number(inv.total || 0).toFixed(2)} ${inv.currencySymbol || "₪"}`);
...
function openPurchaseModal() {
  safeText(qs("purchaseModalTitle"), "إضافة فاتورة شراء");
  safeValue(qs("editPurchaseId"), "");
  safeValue(qs("purchaseSupplier"), "");
  safeValue(qs("purchaseAmount"), "");
  safeValue(qs("purchaseNotes"), "");
...
async function editPurchase(id) {
  ...
  safeText(qs("purchaseModalTitle"), "تعديل فاتورة شراء");
  safeValue(qs("editPurchaseId"), p.id || "");
  safeValue(qs("purchaseSupplier"), p.supplier || "");
  safeValue(qs("purchaseAmount"), p.amount || "");
  safeValue(qs("purchaseNotes"), p.notes || "");
  toggleModal("purchaseModal", true);
}
...
async function toggleScannerTorch() {
  ...
  try {
    ...
    safeText(qs("scannerTorchBtn"), scannerTorchOn ? "إيقاف الفلاش" : "تشغيل / إيقاف الفلاش");
  } catch (err) {
    console.error(err);
...
async function closeScanner(fromPopState = false) {
  ...
  qs("scannerTorchBtn")?.classList.add("hidden");
  qs("scannerTorchQuickBtn")?.classList.add("hidden");
  safeText(qs("scannerTorchBtn"), "تشغيل / إيقاف الفلاش");
...
async function renderReports() {
  ...
  safeText(qs("repWholesaleSales"), money(costs));
  safeText(qs("repTotalSales"), money(sales));
  safeText(qs("repTotalProfit"), money(sales - costs));
  safeText(qs("repPurchases"), money(purchases));
  safeText(qs("repCount"), count);
}
...
async function loadSettingsPage() {
  ...
  safeValue(qs("setStoreName"), store.name || "");
  safeValue(qs("setStoreLogo"), store.logo || "");
  setImageOrHide(qs("settingsLogoPreview"), store.logo);

  safeValue(qs("currencyNameInput"), settings.currencyName || "شيكل");
  safeValue(qs("currencySymbolInput"), settings.currencySymbol || "₪");
...
async function handleCustomerInput() {
  ...
  suggestions.forEach(item => {
    ...
    `;
    div.onclick = () => {
      safeValue(qs("customerName"), item.name || "");
      safeValue(qs("customerPhone"), item.phone || "");
      box.classList.add("hidden");
    };
    box.appendChild(div);
  });
...
function openNoteModal(note) {
  safeText(qs("noteModalContent"), note || "-");
  toggleModal("noteModal", true);
}

function openStatusModal(invoiceId, currentStatus) {
  safeValue(qs("statusInvoiceId"), invoiceId);
  safeValue(qs("statusSelect"), currentStatus || "paid");
...
async function openCustomerHistory(name, phone = "") {
  ...
  safeText(qs("customerHistoryTitle"), `${name || "بدون اسم"}${phone ? " - " + phone : ""}`);
  safeText(qs("custPaidTotal"), money(paid));
  safeText(qs("custUnpaidTotal"), money(unpaid));
  safeText(qs("custGrandTotal"), money(total));
...
async function createAggregateInvoiceForCustomer() {
  ...
  safeValue(qs("customerName"), currentCustomerHistoryName);
  safeValue(qs("customerPhone"), currentCustomerHistoryPhone || "");
  safeValue(qs("invoiceNotes"), `فاتورة مجمعة للعميل - عدد الفواتير: ${customerInvoices.length}`);
  safeValue(qs("invoiceStatus"), "paid");
  cart = [];
  renderCart();
  calculateTotal();

  alert(`إجمالي الفواتير المطابقة: ${money(total)}\nيمكنك الآن إنشاء فاتورة جديدة لهذا العميل.`);
}
...
async function saveEntity(kind, id, payload) {
  ...
  if (isOnline() && session?.appMode === "online" && baseClientPath()) {
    ...
    try {
      ...
      console.warn("Cloud save failed, kept offline:", err);
    }
  }
}
...
async function deleteEntity(kind, id) {
  ...
  if (isOnline() && session?.appMode === "online" && baseClientPath()) {
    ...
    try {
      ...
      console.warn("Cloud delete failed, removed offline:", err);
    }
  }
}
...
async function syncCloudToOffline() {
  ...
  try {
    ...
    console.warn("syncCloudToOffline failed:", err);
  }
}
...
async function loadCurrentStore() {
  const store = await idbGet("stores", currentStoreId);
  if (store) {
    safeText(qs("sideStoreName"), store.name || "اسم المحل");
