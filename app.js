...
function bindBaseEvents() {
  ...
  qs("customerHistoryRange")?.addEventListener("change", () => {
    if (currentCustomerHistoryName) openCustomerHistory(currentCustomerHistoryName);
  });
...
function money(value, withName = false) {
  const cache = getLocalSettingsCache() || {
    currencyCode: "ILS",
    currencySymbol: "₪",
    currencyName: "شيكل"
  ...
  return withName
    ? `${amount} ${cache.currencyName} ${cache.currencySymbol}`
    : `${amount} ${cache.currencySymbol}`;
}

function applyPlanBadgeFromSession() {
  const session = getLocalSession();
  const isUnlimited = session?.durationType === "unlimited";
  const text = isUnlimited ? "نسخة برو" : "النسخة العادية";
  const className = isUnlimited
    ? "px-3 py-1 rounded-full text-xs font-bold bg-violet-100 text-violet-700"
    : "px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700";

  qs("licensePlanBadge").className = className;
  qs("licensePlanBadge").innerText = text;

  qs("settingsPlanBadge").className = className;
  qs("settingsPlanBadge").innerText = text;
}
...
function numericFromMoneyText(text) {
  const only = String(text || "").replace(/[^\d.-]/g, "");
  return parseFloat(only || "0") || 0;
}
...
async function viewInvoice(id) {
  ...
  qs("invPageCurrency").innerText = `${inv.currencyName || "شيكل"} ${inv.currencySymbol || "₪"}`;
...
async function shareCurrentInvoice() {
  ...
  const snap = await get(ref(db, `${pathClientInvoices()}/${currentInvoiceId}`));
  if (!snap.exists()) return;
  const inv = snap.val();
  ...
  visible.forEach(inv => {
    table.innerHTML += `
      <tr class="border-b hover:bg-gray-50">
        ...
        <td class="p-4">
          <button onclick="openCustomerHistory('${escapeJs(inv.customer || "")}')" class="text-blue-700 font-bold hover:underline">
...
async function savePurchase() {
  ...
  const oldSnap = existingId ? await get(ref(db, `${pathClientPurchases()}/${id}`)) : null;
  const oldCreatedAt = oldSnap && oldSnap.exists() ? oldSnap.val().createdAt : null;
...
async function loadSettingsPage() {
  ...
  qs("setStoreName").value = s.name || "";
  qs("setStoreLogo").value = s.logo || "";
  setImageOrHide(qs("settingsLogoPreview"), s.logo);
...
async function buildBackupPayload() {
  const [stores, products, invoices, purchases, settingsSnap] = await Promise.all([
    getAllStores(),
    getAllProducts(),
    getAllInvoices(),
    getAllPurchases(),
...
async function restoreBackupPayload(data) {
  ...
  if (data.settings) {
    ...
    setLocalSettingsCache({
      currencyCode: data.settings.currencyCode || "ILS",
      currencySymbol: getCurrencyMeta(data.settings.currencyCode || "ILS").symbol,
      currencyName: getCurrencyMeta(data.settings.currencyCode || "ILS").name
    });
  }
...
async function saveInvoiceStatus() {
  ...
  await update(ref(db, `${pathClientInvoices()}/${id}`), {
    status,
    updatedAt: new Date().toISOString()
  });
...
async function openCustomerHistory(name) {
  ...
  filtered.forEach(inv => {
    ...
    else if (inv.status === "review") review += t;
    else unpaid += t;
  });

  qs("customerHistoryTitle").innerText = name || "بدون اسم";
  qs("custPaidTotal").innerText = money(paid);
  qs("custUnpaidTotal").innerText = money(unpaid);
  qs("custReviewTotal").innerText = money(review);
