...
const LOCAL_OFFLINE_DB_NAME = `${PREFIX}_offline_db_v2`;
const LOCAL_OFFLINE_DB_VERSION = 2;
const LOCAL_BOOT_DONE_KEY = `${PREFIX}_BOOT_DONE`;
...
function getCurrencyMeta(settings) {
  return {
    name: settings?.currencyName || "شيكل",
...
async function updateCurrencyUI() {
  ...
  qs("sideCurrencyText").innerText = `${settings.currencySymbol} ${settings.currencyName}`;
  ...
  const session = getLocalSession();
  const allowSync = session?.appMode === "online";
  qs("offlineSyncWrap").classList.toggle("hidden", !allowSync);
}
...
async function bootSessionState() {
  ...
  localStorage.setItem(LOCAL_BOOT_DONE_KEY, "1");
}
...
async function refreshSessionFromLicense() {
  ...
  const mergedSettings = {
    id: "settings",
    currencyName: cloudSettings?.currencyName || (await getClientSettings()).currencyName || "شيكل",
    currencySymbol: cloudSettings?.currencySymbol || (await getClientSettings()).currencySymbol || "₪",
    appMode
  };
...
async function loadSettingsPage() {
  ...
  qs("currencyNameInput").value = settings.currencyName || "شيكل";
  qs("currencySymbolInput").value = settings.currencySymbol || "₪";
...
async function createAggregateInvoiceForCustomer() {
  ...
  const statusFilter = qs("customerInvoiceAggregateStatus").value;
  const rangeFilter = qs("customerInvoiceAggregateRange").value;
  ...
  qs("invoiceNotes").value = `فاتورة مجمعة للعميل - عدد الفواتير: ${customerInvoices.length}`;
  qs("invoiceStatus").value = "paid";
  cart = [];
  renderCart();
  calculateTotal();

  alert(`إجمالي الفواتير المطابقة: ${money(total)}\nيمكنك الآن إنشاء فاتورة جديدة لهذا العميل يدويًا أو إضافة بند دفعة.`);
}
...
async function sendDebtMessageToCustomer() {
  ...
  const app = qs("messageTargetApp").value;
  const prefixMode = qs("messageCountryPrefixMode").value;
...
async function exportBulkPurchases(type) {
  const range = qs("bulkPurchasesRange").value;
